...
int main(int argc, char** argv) {
    ...
    // Progress accounting: total pairs = sum(max(0, size-1)) over cameras
    int total_pairs = 0;
    for (const auto &kv : frames_by_cam) {
        const auto &v = kv.second;
        if (v.size() >= 2) total_pairs += int(v.size()) - 1;
    }
    int processed_pairs = 0;
    int next_report_pct = 0;
    auto report_progress = [&](){
        if (total_pairs <= 0) return;
        int pct = (processed_pairs * 100) / std::max(1, total_pairs);
        if (pct >= next_report_pct) {
            std::cout << "[progress] " << processed_pairs << "/" << total_pairs
                      << " (" << pct << "%)" << std::endl;
            next_report_pct = std::min(100, pct + 2); // report ~every 2%
        }
    };
    ...
    // Snapshot / events settings via environment
    int events_every = 0; // write sampled events every N processed pairs; 0 = disabled
    int events_max   = 20000; // max points per snapshot
    std::string events_dir = "outputs/snapshots";
    if(const char* s = std::getenv("SNAPSHOT_EVERY")) { events_every = std::max(0, std::atoi(s)); }
    if(const char* s = std::getenv("SNAPSHOT_MAX"))   { events_max   = std::max(100, std::atoi(s)); }
    if(const char* s = std::getenv("SNAPSHOT_DIR"))   { events_dir   = s; }
    if(events_every > 0) {
        std::error_code ec; std::filesystem::create_directories(events_dir, ec);
    }
    ...
    for(auto &kv : frames_by_cam) {
        ...
        for(size_t i=0; i<cam_frames.size(); i++){
            ...
            // Also sample a subset of touched voxels to write lightweight snapshot events
            std::vector<int> sampled_indices; sampled_indices.reserve((size_t)events_max);
            int sample_stride_counter = 0;
            for(int v = 0; v < mm.height; v++){
                for(int u = 0; u < mm.width; u++){
                    ...
                    for(const auto &rs : steps) {
                        ...
                        voxel_grid[idx] += val;
                        if(events_every > 0) {
                            if(((sample_stride_counter++) & 31) == 0) {
                                if((int)sampled_indices.size() < events_max) sampled_indices.push_back(idx);
                            }
                        }
                    }
                }
            ...
            // Progress: finished one pair (prev -> curr) after first valid
            if (prev_valid) {
                processed_pairs++;
                report_progress();
                if(events_every > 0 && (processed_pairs % events_every) == 0) {
                    json arr = json::array(); arr.reserve(sampled_indices.size());
                    float half_size = 0.5f * (N * voxel_size);
                    for(int idx : sampled_indices) {
                        int iz = idx % N; int t = idx / N; int iy = t % N; int ix = t / N;
                        float xw = -half_size + (ix + 0.5f) * voxel_size + grid_center.x;
                        float yw = -half_size + (iy + 0.5f) * voxel_size + grid_center.y;
                        float zw = -half_size + (iz + 0.5f) * voxel_size + grid_center.z;
                        float val = voxel_grid[idx];
                        arr.push_back({{"x", xw}, {"y", yw}, {"z", zw}, {"v", val}, {"t", processed_pairs}});
                    }
                    char namebuf[64]; std::snprintf(namebuf, sizeof(namebuf), "events_%06d.json", processed_pairs);
                    std::string out_path = events_dir + "/" + std::string(namebuf);
                    std::ofstream jf(out_path);
                    if(jf.is_open()) {
                        jf << arr.dump();
                        jf.close();
                        std::cout << "[snapshot] wrote " << out_path << " (" << sampled_indices.size() << ")\n";
                    }
                }
            }
        }
