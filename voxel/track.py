...
def main():
    ...
    ap.add_argument("--kf-smooth", action="store_true", help="Apply constant-velocity Kalman smoothing on the trajectory")
    ap.add_argument("--kf-q", type=float, default=0.05, help="Process noise scale for KF")
    ap.add_argument("--kf-r", type=float, default=0.5, help="Measurement noise for KF (meters)")
    ap.add_argument("--prior-percentile", type=float, default=None, help="Percentile trim for global prior (e.g., 99.0)")
    ap.add_argument("--mask-blur", type=int, default=0, help="Gaussian blur kernel (odd) for masks before centroid/projection")
    ap.add_argument("--mask-open", type=int, default=0, help="Morphological open kernel size for masks before use")
    args = ap.parse_args()
    ...
    if args.use_global_prior:
        ...
        if global_prior is not None:
            ...
            if args.prior_percentile is not None:
                p = float(args.prior_percentile)
                flat = global_prior.ravel()
                if np.any(flat > 0):
                    thr = np.percentile(flat[flat > 0], p)
                    global_prior = (global_prior >= thr).astype(np.float32)
    ...
    for i in range(min_len):
        ...
        if args.mask_blur and args.mask_blur > 0:
            bsz = int(max(1, args.mask_blur)) | 1
            masks_this = [cv2.GaussianBlur(m, (bsz, bsz), 0) if m is not None else None for m in masks_this]
        if args.mask_open and args.mask_open > 0:
            ker = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (int(args.mask_open), int(args.mask_open)))
            masks_this = [cv2.morphologyEx(m, cv2.MORPH_OPEN, ker) if m is not None else None for m in masks_this]
    ...
    # Optional Kalman smoothing (constant velocity 3D)
    if args.kf_smooth and len(track) >= 2:
        dt = 1.0  # assume unit frame time; adapt if timestamps available
        F = np.eye(6, dtype=np.float64)
        for j in range(3):
            F[j, j+3] = dt
        H = np.zeros((3, 6), dtype=np.float64)
        H[0,0] = 1.0; H[1,1] = 1.0; H[2,2] = 1.0
        Q = (float(args.kf_q) ** 2) * np.eye(6, dtype=np.float64)
        R = (float(args.kf_r) ** 2) * np.eye(3, dtype=np.float64)

        z0 = np.array([track[0]["x"], track[0]["y"], track[0]["z"]], dtype=np.float64)
        if len(track) >= 2:
            z1 = np.array([track[1]["x"], track[1]["y"], track[1]["z"]], dtype=np.float64)
            v0 = (z1 - z0) / max(1e-6, dt)
        else:
            v0 = np.zeros(3, dtype=np.float64)
        xhat = np.concatenate([z0, v0], axis=0)
        ...
        for j in range(len(track)):
            ...
            P = F @ P @ F.T + Q
            # Update
            z = np.array([track[j]["x"], track[j]["y"], track[j]["z"]], dtype=np.float64)
            yk = z - (H @ xhat)
            S = H @ P @ H.T + R
            K = P @ H.T @ np.linalg.inv(S)
            xhat = xhat + K @ yk
            P = (np.eye(6) - K @ H) @ P
            smoothed.append({"t": track[j]["t"], "x": float(xhat[0]), "y": float(xhat[1]), "z": float(xhat[2])})
        track = smoothed
